...
Define the following set $$B := \left\lbrace \mathcal{X} \in \mathcal{M}^{eq}_{\mathbf{n}, \mathbf{r}} : \text{rank}_{CP} \mathcal{X} < q^{\frac{d}{2}} \right\rbrace,$$ where  $q = \text{min} \left\lbrace n, r-1 \right\rbrace$.
